...
- **Specialist:** {agent role from SQUAD_PLANNING.md, e.g., python-architect}
- **Complexity:** {S | M | L}
...
| Good | `[ ] Rate limiter returns 429 after 100 requests in 60s (test: test_rate_limit.py)` |
...
| Quality | Example |
|---------|---------|
| Good | `New file: lib/rate_limiter.py — sliding window counter using Redis INCR` |
...
| Quality | Example |
|---------|---------|
| Good | `tests/test_rate_limiter.py — unit tests: counter increment, window reset, 429 response, header values` |
...
| Quality | Example |
|---------|---------|
| Good | `Update gen_demo.py to include rate_limit_tier field on user docs` |
...
- **Technical Specs:**
  - New file: `lib/rate_limiter.py` — sliding window counter (in-memory dict for local, Redis for production)
  - New middleware: `middleware/rate_limit.py` — wraps route handlers
...
- **Test Plan:**
  ...
  - `tests/test_rate_limit_middleware.py` — integration: header presence, 429 response, reset behavior
- **Demo Data Impact:**
